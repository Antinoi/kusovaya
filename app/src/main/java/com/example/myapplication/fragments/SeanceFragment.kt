...
class SeanceFragment : Fragment(), SeancesAdapter.OnSeanceListener {
    ...
    override fun onGoSeance(seansId: Long) {
        if(itemModel.findSeanceToGo(requireContext(), arguments?.getLong("userId")!!, seansId) == Unit){
            itemModel.go(requireContext(), arguments?.getLong("userId")!!, seansId )
        }else{

                val toast = Toast.makeText(requireContext(), "Вы уже идёте на этот сеанс", Toast.LENGTH_SHORT)
                toast.show()
